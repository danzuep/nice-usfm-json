...
public class TextParser : IUsxElementParser
{
    ...
    {
        if (usjChar.Content == null || usjChar.Content.Count != 1 || usjChar.Content[0] is not UsjText usjText)
        {
            return usjChar;
        }
        var usjNodes = ParseEnriched(usjText.Text, false);
        if (!usjNodes.Any())
        {
            return usjChar;
        }
        return usjChar with { Content = usjNodes };
    }
    ...
    public IList<IUsjNode> ParseEnriched(string? text, bool addTextAfterClear = true)
    {
        ...
        foreach (var rune in text.EnumerateRunes())
        {
            ...
            else
            {
                ...
                usjNodes.Add(new UsjChar(Content: new List<IUsjNode> { new UsjText(rune.ToString()) }, Style: "rune"));
            }
        }
